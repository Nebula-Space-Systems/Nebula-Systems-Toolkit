...
def _min_asset_empirical_kernel(
    obs_stack: np.ndarray,  # (nt, n_obs, 3)
    min_counts: np.ndarray,  # (ny, nx) int32 (in/out)
    row_remaining: np.ndarray,  # (ny,) int32 (in/out) number of cells not yet at 0 in each row
    ...
    # Evaluate at every provided sample; min() does not depend on dt.
    for ti in range(nt):
        ...
        # parallel over rows
        for j in prange(ny):
            # If this entire row has already reached 0 everywhere, it can never improve.
            ...
            # prefix-sum counts; update per-cell minimum count
            run = 0
            row_min = min_counts[j]
            rem = row_remaining[j]

            for i in range(nx):
                run += diff_row[i]  # instantaneous count at (j,i)

                # Only update if we found a smaller instantaneous count than previously seen.
                if run < row_min[i]:
                    row_min[i] = run

                    # If it just reached 0, reduce remaining count; enables future early-exit.
                    if run == 0:
                        rem -= 1
...
def calculate_min_asset(
    ...
    """
    min_asset metric: for each grid cell, return the minimum instantaneous number of
    ...
    - This metric does NOT use time deltas. It evaluates visibility at each provided
      sample index (ti = 0..nt-1) and takes the minimum count over those samples.
    - If a grid cell ever sees 0 assets at any sample, its result becomes 0.
    ...
    # Initialize to maximum possible; kernel reduces via min.
    min_counts = np.full((ny, nx), n_obs, dtype=np.int32)

    # Track how many cells per row have not yet hit 0; enables early exit per row.
    row_remaining = np.full(ny, nx, dtype=np.int32)

    # Scratch: diff grid per row (nx+1), signed
    diff_grid = np.zeros((ny, nx + 1), dtype=np.int32)

    _min_asset_empirical_kernel(
        obs_stack,
        min_counts,
        row_remaining,
        diff_grid,
