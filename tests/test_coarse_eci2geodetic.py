...
from nebula.transforms._coarse_eci2geodetic import (
    ...
    _coarse_eci2geodetic_vec_iau76_shortnut,
    coarse_eci2geodetic,
...
def test_coarse_eci2geodetic_scalar_matches_composed_path() -> None:
    ...
    for i in range(r_eci.shape[0]):
        ...
        la, lo, h = _coarse_eci2geodetic_iau76_shortnut(
            float(r_eci[i, 0]),
            float(r_eci[i, 1]),
            float(r_eci[i, 2]),
            float(jd_ut1[i]),
...
def test_coarse_eci2geodetic_deg_scalar_matches_composed_path() -> None:
    ...
    for i in range(r_eci.shape[0]):
        ...
        la, lo, h = _coarse_eci2geodetic_deg_iau76_shortnut(
            float(r_eci[i, 0]),
            float(r_eci[i, 1]),
            float(r_eci[i, 2]),
            float(jd_ut1[i]),
...
def test_coarse_eci2geodetic_vector_matches_scalar() -> None:
    ...
    lat_v, lon_v, h_v = _coarse_eci2geodetic_vec_iau76_shortnut(r_eci, jd_ut1, jd_tt)
...
def test_coarse_eci2geodetic_vector_deg_matches_scalar_deg() -> None:
    ...
    lat_v, lon_v, h_v = _coarse_eci2geodetic_vec_deg_iau76_shortnut(
        r_eci, jd_ut1, jd_tt
    )
    ...
    h_s = np.empty_like(h_v)
    for i in range(r_eci.shape[0]):
        la, lo, hi = _coarse_eci2geodetic_deg_iau76_shortnut(
            float(r_eci[i, 0]),
            float(r_eci[i, 1]),
            float(r_eci[i, 2]),
            float(jd_ut1[i]),
...
def test_public_aliases_exported_from_transform_namespace() -> None:
    ...
    assert callable(transforms.coarse_eci2geodetic_deg)
    assert callable(transforms.coarse_eci2geodetic_vec)
    assert callable(transforms.coarse_eci2geodetic_vec_deg)


def test_simple_public_names_match_legacy_names() -> None:
    r_eci, jd_ut1, jd_tt = _random_eci_samples(64, seed=8)

    lat0, lon0, h0 = coarse_eci2geodetic_vec(r_eci, jd_ut1, jd_tt)
    lat1, lon1, h1 = transforms.coarse_eci2geodetic_vec(r_eci, jd_ut1, jd_tt)
    ...
    d1 = transforms.coarse_eci2geodetic_deg(
        float(r_eci[i, 0]),
        float(r_eci[i, 1]),
        float(r_eci[i, 2]),
        float(jd_ut1[i]),
    ...
    vd0 = coarse_eci2geodetic_vec_deg(r_eci, jd_ut1, jd_tt)
    vd1 = transforms.coarse_eci2geodetic_vec_deg(r_eci, jd_ut1, jd_tt)
